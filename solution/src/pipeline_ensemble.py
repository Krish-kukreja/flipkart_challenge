...
val_preds_lgb = np.clip(model_lgb.predict(X_val), 0, 1)
val_preds_xgb = np.clip(model_xgb.predict(X_val), 0, 1)
val_preds_cb = np.clip(model_cb.predict(X_val), 0, 1)

# simple average ensemble
val_preds = (val_preds_lgb + val_preds_xgb + val_preds_cb) / 3.0
cv_r2 = r2_score(y_val, val_preds)
print(f"Ensemble CV R2: {cv_r2:.5f}")
...
model_lgb.fit(X_final, y_final, categorical_feature=cat_features)
model_xgb.fit(X_final, y_final, verbose=False)
model_cb.fit(X_final, y_final, cat_features=cat_features)

print("Predicting on test set...")
final_preds_lgb = np.clip(model_lgb.predict(X_test), 0, 1)
final_preds_xgb = np.clip(model_xgb.predict(X_test), 0, 1)
final_preds_cb = np.clip(model_cb.predict(X_test), 0, 1)

# Simple average ensemble
final_preds = (final_preds_lgb + final_preds_xgb + final_preds_cb) / 3.0
